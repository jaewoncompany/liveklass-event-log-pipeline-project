...
데이터 파이프라인 구축은 처음 도전해본 영역이라 구현하면서 각 개념을 하나씩 이해하는 데 집중했습니다. Grafana는 평소에 관심 있게 공부했던 도구라 로컬 환경에서 Provisioning 자동화까지 구현했습니다. 선택 과제에서는 로컬 스택을 AWS로 옮기는 과정에서 S3+Athena가 이벤트 로그에 적합한 이유, Kinesis가 필요한 이유, 그리고 시각화 도구로 QuickSight를 선택한 이유를 직접 고민하며 설계했습니다.
