...
### `etl_checkpoint`
micro-batch 처리 시 마지막으로 처리한 `raw_id`를 기록합니다. transform과 checkpoint 업데이트를 하나의 트랜잭션으로 처리해서, 중간에 실패해도 해당 청크부터 재처리할 수 있습니다. `raw_id UNIQUE` 제약으로 재실행 시 중복 적재도 방지합니다.
...
> 현재 파티션은 2026년 4~6월만 정의돼 있고 DEFAULT PARTITION으로 범위 밖 데이터를 처리합니다. 운영 환경에서는 월별 파티션 자동 생성이 필요합니다.
...
이벤트 생성기는 실행 후 종료되는 구조라 Deployment 대신 CronJob만 사용합니다.
...
데이터 파이프라인 구축은 처음 도전해본 영역이라 구현하면서 각 개념을 하나씩 이해하는 데 집중했습니다. 로컬 환경에서는 평소 관심 있게 공부했던 Grafana를 활용해 Provisioning 자동화까지 구현했고, AWS 아키텍처에서는 IAM 기반 접근 제어와 비개발자 친화적인 리포트 공유가 가능한 QuickSight를 시각화 도구로 선택했습니다. 선택 과제를 통해 로컬 스택을 AWS로 옮기는 과정에서 각 서비스의 역할과 선택 이유를 직접 고민하며 설계했습니다.
